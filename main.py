...
async def start_cmd(message: types.Message):
    ...
    await message.answer("⚡️ **ИИ Риелтор активирован**.\n\nДобро пожаловать в Личный Кабинет единой платформы недвижимости всей РФ. Выберите режим работы системы:", reply_markup=builder.as_markup())
...
async def set_slot_index(callback: types.CallbackQuery):
    ...
    builder = InlineKeyboardBuilder()
    builder.row(types.InlineKeyboardButton(text="Московская область", callback_data="geo_reg_Moscow"))
    builder.row(types.InlineKeyboardButton(text="Краснодарский край (Сочи)", callback_data="geo_reg_Sochi"))
    builder.row(types.InlineKeyboardButton(text="Республика Татарстан", callback_data="geo_reg_Kazan"))
    builder.row(types.InlineKeyboardButton(text="⬅️ В меню", callback_data="back_to_main"))
    await callback.message.edit_text("🌍 **КАСКАДНЫЙ ФИЛЬТР РФ: ШАГ 1**\n\nВыберите интересующий Субъект Российской Федерации:", reply_markup=builder.as_markup())
...
async def geo_reg_callback(callback: types.CallbackQuery):
    ...
    region_name = "Московская обл." if reg_key == "Moscow" else "Краснодарский край" if reg_key == "Sochi" else "Татарстан"
    USER_STATES[callback.from_user.id]["region"] = region_name
    builder = InlineKeyboardBuilder()
    if reg_key == "Moscow":
        builder.row(types.InlineKeyboardButton(text="Москва", callback_data="geo_cit_Msk"))
        builder.row(types.InlineKeyboardButton(text="Орехово-Зуево", callback_data="geo_cit_Oz"))
    elif reg_key == "Sochi":
        builder.row(types.InlineKeyboardButton(text="Сочи", callback_data="geo_cit_Sch"))
        builder.row(types.InlineKeyboardButton(text="Краснодар", callback_data="geo_cit_Krd"))
    else:
        builder.row(types.InlineKeyboardButton(text="Казань", callback_data="geo_cit_Kzn"))
    await callback.message.edit_text(f"📍 **РЕГИОН**: {region_name}\n\n**ШАГ 2**: Выберите город внутри этого субъекта:", reply_markup=builder.as_markup())

@router.callback_query(lambda c: c.data.startswith("geo_cit_"))
async def geo_cit_callback(callback: types.CallbackQuery):
    cit_key = callback.data.split("_")[2]
    city_name = "Москва" if cit_key == "Msk" else "Орехово-Зуево" if cit_key == "Oz" else "Сочи" if cit_key == "Sch" else "Краснодар" if cit_key == "Krd" else "Казань"
    ...
    if city_name == "Сочи":
        ...
        builder.row(types.InlineKeyboardButton(text="Весь город / Все районы", callback_data="geo_dst_Все"))
    await callback.message.edit_text(f"🏙️ **ГОРОД**: {city_name}\n\n**ШАГ 3**: Выберите интересующий микрорайон/округ:", reply_markup=builder.as_markup())
...
async def budget_callback(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text("⏳ *Конфигурация слота сохранена в xo_base.db. Запускаю сквозной перехват ссылок...*")
